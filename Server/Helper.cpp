...
char* Helper::getPartFromSocket(int sc, int bytesNum)
{
    ...
    if (res == -1)  // Change INVALID_SOCKET to -1
    {
...
void Helper::sendData(int sc, std::string message)
{
    ...
    if (send(sc, data, binStr.size(), 0) == -1)  // Change INVALID_SOCKET to -1
    {
...
std::string Helper::getAllTheSocket(int sc)
{
    ...
    std::string asci = convertToAscii(buffer);
    if (asci[0] == '\0')
...
string Helper::base64_encode(const unsigned char* data, size_t len)
{
...
string Helper::base64_decode(const std::string& encoded)
{
	const std::string base64_chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
